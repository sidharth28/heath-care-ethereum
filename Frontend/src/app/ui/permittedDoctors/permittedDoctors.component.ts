...
export class PermittedDoctorsComponent {
  ...
  constructor(private contract: ContractService, private snackbar: MdcSnackbar) {
    ...
    contract.seeAccountInfo().then((value: any) => {
      ...
      this.contract.getDoctersWithCreateOnlyAccess(this.direction).then((r) => {
        console.log("getDoctersWithCreateOnlyAccess", r)
        this.doctorsWithCreatOnlyAccess = r;

        for (let i = 0; i < this.doctorsWithCreatOnlyAccess.length; i++) {

          this.contract.getDoctersDetails(this.direction, this.doctorsWithCreatOnlyAccess[i]).then((rx) => {
            console.log(rx);
            rx['access'] = "Create";
      ...
      this.contract.getDoctersWithViewOnlyAccess(this.direction).then((r) => {
        console.log("getDoctersWithViewOnlyAccess", r)
        this.doctorsWithViewOnlyAccess = r;

        for (let i = 0; i < this.doctorsWithViewOnlyAccess.length; i++) {

          this.contract.getDoctersDetails(this.direction, this.doctorsWithViewOnlyAccess[i]).then((rx) => {
            console.log(rx);
            rx['access'] = "View";
